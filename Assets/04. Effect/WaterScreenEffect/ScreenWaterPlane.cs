...
public class ScreenWaterPlane : MonoBehaviour
{
    ...
    private const string FeatureName = "FullScreenWater"; // Renderer Feature 이름
    ...
    // LerpColor 설정(알파는 코루틴에서 제어)
    [SerializeField] private Color lerpBaseColor = new Color(0.2431f, 0.2784f, 1f, 1f); // RGB만 사용
    [SerializeField] private float lerpAlphaMax = 0.35f; // 셰이더가 0~100 스케일이면 35로 설정
    ...
    void Start()
    {
        ...
        {
            Debug.LogWarning($"[ScreenWaterPlane] '{FeatureName}' 머티리얼을 찾을 수 없습니다.");
        }
        // 초기값(선택)
    ...
    private System.Collections.IEnumerator Co_Splash()
    {
        ...
        // 평면 머티리얼 빠른 페이드 인(팝 방지) + LerpColor 알파도 함께 상승
        float tAttack = 0f;
        ...
        while (t < total)
        {
            ...
            // Opacity: 1 -> 0 (startOpacityFade 시점부터)
            if (m_WaterPlaneMaterial != null && m_WaterPlaneMaterial.HasProperty(m_WaterPlaneOpacityProperty))
            {
            ...
            // LerpColor 알파: 피크(lerpAlphaMax) -> 0 (동일 시점에 릴리즈)
            if (m_WaterPlaneMaterial != null && m_WaterPlaneMaterial.HasProperty(m_WaterPlaneLerpColorProperty))
            {
